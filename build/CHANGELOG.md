...
```
FASE: 0 · Planning (pre-build)
S0 Cimientos:        ⬜ no iniciado   (bloqueante — corre primero)
A  Contratos:        ⬜ bloqueado por S0
B  Economía agentes: ⬜ bloqueado por S0
C  Reputación chain: ⬜ bloqueado por S0
MERGE Integración:   ⬜ bloqueado por A+B+C
FRONT (otra persona):⬜ se acopla después (repo aparte)
ÚLTIMO PASO CERRADO: —
PRÓXIMO PASO: abrir Sesión 0 (cimientos) → mergear a main
BLOQUEOS: ninguno
```
...
- [ ] Scaffold (`contracts/`, `backend/lib/`, `backend/agents/`, `abi/`) + `package.json` + `tsconfig`
- [ ] `.env.example` + `backend/lib/{env,chain,types}.ts`
- [ ] `backend/lib/reputation.ts` (interfaz `ReputationLayer` + `MockReputationLayer`)
- [ ] `abi/ISCoreRegistry.json` + `abi/IReputationSBT.json` + `abi/deployments.json` (placeholders)
- [ ] **Merge a `main`** → habilita A/B/C
